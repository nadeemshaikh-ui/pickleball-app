...
// Knockout visualization — genuinely new component, no existing analog in
// the app. Groups matches into columns by bracket_round; each round's column
// gets progressively more top/bottom margin per match so pairs visually
// funnel toward the next round without needing an SVG/canvas connector
// library — a standard CSS-only bracket trick.
export default function TournamentBracketTree({ matches, teams, onScoreClick }: TournamentBracketTreeProps) {
  ...
  return (
    <div style={{ display: 'flex', gap: 24, overflowX: 'auto', padding: '8px 0' }}>
      {roundNumbers.map(round => {
        ...
        const gapMultiplier = 2 ** round; // doubles the vertical gap each round so pairs funnel visually
        return (
          <div key={round} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 16 * gapMultiplier, minWidth: 180 }}>
            <div style={{ textAlign: 'center', fontSize: 12, fontWeight: 800, color: 'var(--muted)' }}>
