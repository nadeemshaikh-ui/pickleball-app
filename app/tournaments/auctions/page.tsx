...
export default function AuctionsPage() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setNewName('');
      window.location.href = `/tournaments/auctions/${id}`;
    } catch (e) {
  ...
  return (
    <main className="page">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {auctions.map(a => (
          <Link key={a.id} href={`/tournaments/auctions/${a.id}`} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
