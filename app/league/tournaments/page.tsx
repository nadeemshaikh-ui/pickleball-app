...
export default function TournamentsPage() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      window.location.href = `/league/tournaments/${id}`;
    } catch (e) {
  ...
  return (
    <main className="page">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {tournaments.map(t => (
          <Link key={t.id} href={`/league/tournaments/${t.id}`} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
